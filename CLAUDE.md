...
- **出欠はカードを持たない。** 2026-08-24 に4枚目のカード（`#after-drop` の段）を削除した。
  理由は「単体では始められないから」**ではない**——出欠システムには「＋ 手で入力して追加」が
  あり、イベントドロッパーの最初の画面にも直接の入口がある（2026-09-03 に食い違いが見つかり、
  `tool1.attendNote` を直した）。カードを持たせない理由は、**主な道が「読み取って、そのまま
  渡す」ほうだから。** 4枚並べると、いちばん楽な道が3枚の中に埋もれる。
...
- OG画像: `assets/og/og-{ja,en,in}.png`（1200×630）。パスを変えるなら `build_seo_head()` も直す。
  - **日本語版は 2026-09-24 に「4本を並べた形」に作り替えた**（X の固定ポストで4本を紹介したら、
    その下に「イベント文書をドロップするだけ」と出ていた）。**作り直すときは `tools/make-og-ja.py`**
    （前の画像は作り方が残っておらず、一から描き直した）
  - 英語版・インド版は**前のまま**（イベントドロッパーだけの頃の絵）。申込書ドロッパーは日本語のみなので、
    作り替えるなら3本の形になる
  - **★ 同じファイル名で差し替えても、X や LINE はしばらく古い画像を出す**（ページのカードを覚えている）
...
2026-07-21 に `calendar.events`（機密スコープ）の本番審査に**承認済み**（JST 23:25、Gmail の実物で確認）。
